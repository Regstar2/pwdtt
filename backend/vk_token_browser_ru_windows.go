...
func (s *vkEdgeSession) obtainModernRUAuthorizeViaBrowser(ctx context.Context) (vkLegacyToken, error) {
	...
	for {
		...
		// Match python273/vk_api more closely: Edge is only used to establish
		// the browser login session and the login.vk.ru `p` cookie. Once that
		// cookie exists, perform authorize -> connect_internal ->
		// auth.getOauthToken on one http.Client/cookie jar without breaking the
		// session between steps. A return_auth hash generated by one HTTP
		// session must not be exchanged by another.
		if hasP && !triedHTTPAfterPCookie {
			triedHTTPAfterPCookie = true
			token, httpErr := s.obtainModernRUTokenSingleHTTPSession(waitCtx)
			if httpErr == nil {
				return token, nil
			}
			return vkLegacyToken{}, fmt.Errorf("VK .ru single-session OAuth: %w (%s)", httpErr, lastDiag)
		}
...
func (s *vkEdgeSession) obtainModernRUTokenSingleHTTPSession(ctx context.Context) (vkLegacyToken, error) {
	client, err := s.newModernRUVKHTTPClient(ctx)
	if err != nil {
		return vkLegacyToken{}, err
	}
	return obtainModernRUTokenWithClient(ctx, client)
}

func obtainModernRUTokenWithClient(ctx context.Context, client *http.Client) (vkLegacyToken, error) {
	if client == nil {
		return vkLegacyToken{}, errors.New("VK .ru HTTP client не настроен")
	}
	...
	if returnAuthHash == "" {
		return vkLegacyToken{}, fmt.Errorf(
			"VK .ru authorize не вернул window.init.data.hash.return_auth (%s)",
			safeModernRUHTTPDiag(finalURL, status, false),
		)
	}

	return exchangeModernRUReturnAuthHashWithClient(ctx, client, returnAuthHash)
}
...
func exchangeModernRUReturnAuthHashWithClient(ctx context.Context, client *http.Client, returnAuthHash string) (vkLegacyToken, error) {
