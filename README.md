...
В текущей ветке `main` реализована автоматическая генерация VK call hashes для Windows через VK Calls API. Функция планируется к выпуску в `v1.6.0`; опубликованный `v1.5.2` её ещё не содержит.
...
Windows-приёмка автоматической генерации VK call hashes завершена: реальные хеши создаются через `calls.start`, сохраняются в профиль и подтверждены рабочим подключением с трафиком. Следующий релиз — `v1.6.0`.
...
- Автоматическое создание VK call hashes доступно в текущем `main` только на Windows и требует Microsoft Edge; в опубликованном `v1.5.2` ещё используется ручной ввод.
