...
- правило блокирует весь исходящий IPv6 через Windows Firewall keyword `Any6` на время активного IPv4-only full-tunnel;
- исходный IPv6 провайдера не используется для пользовательского трафика.

Защита включается только когда `AllowedIPs` содержит IPv4 default route `0.0.0.0/0`. Она не привязана к именам сетевых интерфейсов и не использует CIDR `::/0`: применяется встроенный IPv6 keyword `Any6`, что исключает ошибки Windows Firewall с локализованными alias и неподдерживаемым адресным представлением. Для split-tunnel конфигураций защита не применяется.
